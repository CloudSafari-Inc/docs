...
<ParamField body="trip_id" type="number">
  The unique identified for the trip.
</ParamField>
...
The field returned are the same as in data from the (Get All Trips Response)[/trips/get-all-trips#response-body-parameters] endpoint 
with the exception that it doesn't return an offset number or an array of trip objects, just a singular trip object.
